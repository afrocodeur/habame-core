(function (global, factory) {
    ...
    const ARRAY_OVERRIDABLE_METHODS = ['push', 'pop', 'shift', 'unshift', 'sort', 'reverse', 'splice'];
    ...
    const State = function ($defaultValues = {}, HabameCore) {
      ...
      this.onUpdate = function (names, listener, isToHandleFirst = false) {
        ...
        notFoundStateNames.forEach(name => {
          ...
          const item = dependedStates.find(({
            state
          }) => state === dependedState);
          if (!item) {
            dependedStates.push({
    ...
    const AbstractTemplate = function ($viewProps) {
      ...
      this.getRequestedVars = function (template, stateOnly = true) {
        const states = [];
        const actions = [];
        const possibleRequestedVars = template.replace(/('.*?')|(".*?")/g, '').match(/[a-z_0-9$_.]+/ig)?.map(varCall => {
    ...
    const Template = function ($template, $viewProps, $isWithTry = false, $catchValue = false) {
      ...
      this.refresh = function (sourceTemplate = null, cleanState = false) {
        ...
        try {
          ...
          $templateFunction = new Function('states', (requestedVariablesNames.length ? 'const {' + requestedVariablesNames.join(',') + '} = states;' : '') + returnCode);
        } catch (e) {
    ...
    const TextTemplateDescription = function ($template, $viewProps) {
      ...
      this.refresh = function (template) {
        ...
        $parts = (template || $template).split(/(\{\{.+?\}\})/).map(value => {
     ...
     * @class
     */
    const AbstractViewDev = function ({
      $viewDescription,
      $callbacks
    }) {
      const {
     ...
     * @class
     */
    const ViewTextElementDev = function ({
      $viewDescription,
      ...
      this.refresh = function (template) {
        ...
        try {
          ...
          const {
            states,
            actions
          } = $requestedVariablesNames;
          $actionFunctionBridge = new Function('state', 'states', 'actions', '$event', '$args', (states.length ? 'const {' + states.join(',') + '} = state.getValues(states);' : '') + (actions.length ? 'const {' + actions.join(',') + '} = actions;' : '') + returnCode);
          $lastTemplate = template;
        } catch (e) {
          throw new Error('Syntax error : ' + template);
        }
     ...
     * @class
     */
    const ViewComponentElementDev = function ({
      $viewDescription,
    ...
    const ViewComponentElement = function ($viewDescription, $viewProps) {
      ...
      const buildEvent = function (eventName, actionName) {
        ...
        $componentEventActions[eventName] = {
     ...
     * @class
     */
    const ViewHtmlElementAttributeDev = function ({
      $templateDescription
    }) {
      this.disconnect = function () {
        this.emitUpdate = () => {};
    ...
    const Directive = function ($node, $directives, $attributeTemplates, $viewProps, $lifecycleListeners) {
      ...
      (function () {
        for (const directiveName in $directives) {
          ...
          const directive = $viewProps.appInstance.createDirectiveInstance(directiveName, [{
            element: $node,
            attribute: attrValue,
            attrs: $attributeTemplates
          }]);
     ...
     * @class
     */
    const ViewHtmlElementDev = function ({
      $viewDescription,
      $htmlAttributes,
      $viewProps,
      $htmlEventsStore,
      ...
      this.updateAttributes = function (attributes) {
        ...
        for (const attrName in newAttributes) {
          ...
          $htmlAttributes[attrName] = new ViewHtmlElementAttribute(htmlNode, attrName, newAttributes[attrName], $viewProps);
        }
    ...
    const ViewHtmlElement = function ($viewDescription, $viewProps) {
      ...
      const buildAttrs = function (isUpdateAttribute = true) {
        ...
        for (const attrName in $viewDescription.attrs) {
          $htmlAttributes[attrName] = new ViewHtmlElementAttribute($htmlNode, isUpdateAttribute ? attrName : '', $viewDescription.attrs[attrName], $viewProps);
        }
      };
    ...
    const AbstractLoopExpressionHandler = function ($regex, $regexWithParenthesis) {
      ...
      const $loopExpressionDescription = {
        iterableName: null,
        iterable: null,
        itemValueName: null,
        itemKeyName: null
    ...
    const LOOP_TEMPLATE_HANDLERS = [LoopForInExpressionHandler, LoopForAsExpressionHandler];
     ...
     * @class
     */
    const ViewLoopFragmentDev = function ({
      $loopTemplate,
      $keyTemplate,
      ...
      const updateExistingNodes = function (viewWithoutRepeat) {
        Object.values($nodeInstancesByKey.store).forEach(({
          node
        }) => {
          if (viewWithoutRepeat.name && $viewDescriptionWithoutRepeat.name === viewWithoutRepeat.name || viewWithoutRepeat.component && $viewDescriptionWithoutRepeat.component === viewWithoutRepeat.component) {
    ...
    const ViewLoopFragment = function ($viewDescription, $viewProps) {
      ...
      const $viewDescriptionWithoutRepeat = {
        ...$viewDescription
      };
      ...
      let $itemKeyName = '';
      let $itemValueName = '';
      let keyState = new State({
        [$itemKeyName]: ''
      });
      keyState.parent = $viewProps.getStateToUse();
      const $keyTemplate = new Template('', {
        ...$viewProps,
        localState: keyState
      });
      let $isBuild = false;
      ...
      const updateIterationItem = function (iterable, index) {
        ...
        localState.parent = $viewProps.getStateToUse();
        const node = new ViewElementFragment($viewDescriptionWithoutRepeat, {
          ...$viewProps,
          localState
        });
     ...
     * @class
     */
    const ViewElementFragmentDev = function ({
      $viewDescription,
      $fragmentElements,
      $callbacks
    }) {
      ...
      this.updateViewDescription = function (viewDescription, parentNode) {
        const isViewDescriptionTypeAreDifferent = typeof $viewDescription !== typeof viewDescription || typeof $viewDescription === 'object' && Array.isArray($viewDescription) === Array.isArray(viewDescription);
    ...
    const chimera_directives = {
      ...
      native: ['^if$', '^repeat$', '^else$', '^elseif$', '^props\.', '^events\.', '^directives\.'],
      angular: {
     ...
     * @returns {object|string}
     */
    function chimeraEngine(viewSource) {
    ...
    const App = function (htmlNodeElement, HabameCore) {
      ...
      this.render = function (componentFactory) {
        const instance = typeof componentFactory === 'string' ? this.createComponentByName(componentFactory, null) : createComponentInstance(componentFactory, null);
    ...
    function DynamicComponent(Habame) {
      Habame.createComponent('DynamicComponent', function ({
        State,
        Props,
        App,
        Lifecycle
    ...
    function Link(Habame) {
      Habame.createComponent('Link', function ({
        State,
        Props,
        Actions
      }) {
    ...
    function Router$1(Habame) {
      Habame.createComponent('Router', function ({
        Props,
        State,
        Refs,
        App,
    ...
    const HabameRouter = function (Habame) {
      ...
      return {
        ...
        createRouter: function (name, options) {
          ...
          Habame.createService(name, RouterService, {
            params: [router, history]
          });
          $routers[name] = router;
          return router;
        },
        ...
        createBrowserRouter: function () {
          return Habame.Router.createRouter(null, {
            browserHistory: true
          });
        },
        /**
